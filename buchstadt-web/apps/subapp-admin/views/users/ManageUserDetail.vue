...
import { queryAllAddressByUid, updateOne, deleteOne, updateDefault, insertOne } from "@subapp-admin/apis/api-address";
...
async function handleSubmitUpdateAddress() {
  const { data } = await updateOne(modifiedFormData.value, uid);
  if (data.status === 200) isModify.value = !isModify.value;
}

async function fetchData() {
  const pageRes = await queryAllAddressByUid(uid, pageSize.value, currPage.value);
